...
public class ReviewServiceTest {
    ...
    // Tests save() - Actualizar existente
    ...
    // Tests getAverageRating()

    @Test
    public void testGetAverageRating_CalculaPromedioCorrectamente() {
        // DADO: 3 reseñas con ratings 5, 4, 3 (promedio = 4.0)
        Review r1 = new Review(1L, 10L, 1L, 5, "Excelente", LocalDate.now());
        Review r2 = new Review(2L, 10L, 2L, 4, "Muy bueno", LocalDate.now());
        Review r3 = new Review(3L, 10L, 3L, 3, "Regular", LocalDate.now());
        when(reviewRepository.findByProductId(10L)).thenReturn(Arrays.asList(r1, r2, r3));

        // CUANDO: calculamos el promedio
        Double promedio = reviewService.getAverageRating(10L);

        // ENTONCES: el promedio es 4.0
        assertEquals(4.0, promedio);
    }

    @Test
    public void testGetAverageRating_ProductoSinResenas_RetornaNull() {
        // DADO: producto sin reseñas
        when(reviewRepository.findByProductId(999L)).thenReturn(Arrays.asList());

        // CUANDO: calculamos el promedio
        Double promedio = reviewService.getAverageRating(999L);
    ...
    @Test
    public void testGetAverageRating_UnaResenaSola() {
        // DADO: una sola reseña con rating 5
        Review r1 = new Review(1L, 10L, 1L, 5, "Perfecto", LocalDate.now());
        when(reviewRepository.findByProductId(10L)).thenReturn(Arrays.asList(r1));

        // CUANDO: calculamos el promedio
        Double promedio = reviewService.getAverageRating(10L);

        // ENTONCES: el promedio es 5.0
    ...
    public void testFindByUserId_RetornaResenasDelUsuario() {
        // DADO: 2 reseñas del usuario 5
        Review r1 = new Review(1L, 10L, 5L, 5, "Producto A", LocalDate.now());
        Review r2 = new Review(2L, 20L, 5L, 4, "Producto B", LocalDate.now());
        when(reviewRepository.findByUserId(5L)).thenReturn(Arrays.asList(r1, r2));

        // CUANDO: buscamos por userId
        List<Review> resultado = reviewService.findByUserId(5L);
    ...
    @Test
    public void testFindByUserId_UsuarioSinResenas() {
        // DADO: usuario sin reseñas
        when(reviewRepository.findByUserId(999L)).thenReturn(Arrays.asList());

        // CUANDO: buscamos por userId
        List<Review> resultado = reviewService.findByUserId(999L);
